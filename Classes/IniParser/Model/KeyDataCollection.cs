...
namespace IniParser.Model
{
    ...
    public class KeyDataCollection : ICloneable, IEnumerable<KeyData>
    {
        ...
        public string this[string keyName]
        {
            ...
            set
            {
                if (!_keyData.ContainsKey(keyName.ToLower()))
                {
					this.AddKey(keyName.ToLower());
                }

                _keyData[keyName.ToLower()].Value = value;
        ...
        public bool AddKey(string keyName)
        {
            if ( !_keyData.ContainsKey(keyName.ToLower()) )
            {
                _keyData.Add(keyName.ToLower(), new KeyData(keyName));
                return true;
            }
        ...
        {
            if (AddKey(keyName.ToLower()))
            {
                _keyData[keyName.ToLower()] = keyData;
                return true;
            }
        ...
        public bool AddKey(string keyName, string keyValue)
        {
            if (AddKey(keyName.ToLower()))
            {
                _keyData[keyName.ToLower()].Value = keyValue;
                return true;
            }
        ...
        {
            return _keyData.ContainsKey(keyName.ToLower());
        }
        ...
        {
            if (_keyData.ContainsKey(keyName.ToLower()))
                return _keyData[keyName.ToLower()];
            return null;
        }
        ...
        {
            return _keyData.Remove(keyName.ToLower());
        }
        /// <summary>
        ...
        public void SetKeyData(KeyData data)
        {
            if (data != null)
            {
                if (_keyData.ContainsKey(data.KeyName.ToLower()))
                    RemoveKey(data.KeyName);

                AddKey(data.KeyName.ToLower(), data);
            }
        }
        ...
        public IEnumerator<KeyData> GetEnumerator()
        {
            foreach ( string key in _keyData.Keys )
                yield return _keyData[key.ToLower()];
        }
